...
%
% Last modified by:  Anatoly Yakovlev & Vincent Lee December 14, 2017
%
%  Added new HRV features
...
if (length(indx2) >= 12)
    ...
    rmssd_RR    = rms(diff(A(:,1)));                     % RMS of successive differences of RR intervals
    x_th        = 1.04*m_RR;                              % x definition for pNNx
    pNNx_RR     = sum(diff(A(:,1)) > x_th)/length(diff(A(:,1))); % Percent of differences greater than x (x_th)
%     sdsd_RR     = std(diff(A(:,1)));                     % SD of differences of RR intervals
    
    PCG_seg = zeros(1+max(A(:,2)-A(:,1)), size(A, 1));
    for i=1:size(A,1)-1
        ...
        PCG_seg(:, i) = PCG(A(i,1):A(i,1)+max(A(:,2)-A(:,1)));
    end
    
    rho             = corr(PCG_seg);  % cross - correlation of RR intervals
    xrho            = 0.25;            % x for rhoGTx
    rhoGTx          = mean(sum(rho > xrho)/size(rho,1)); % percentage of RR intervals with correlation grater than xrho
    
    B               = 10;             % Number of bins for histogram of RR intervals
    [N, Edges]      = histcounts(diff(A(:,1)), B, 'Normalization', 'probability');
    idx_nz          = find(N ~= 0);
    entr            = -sum(N(idx_nz).*log(N(idx_nz)));% Shannon entropy of N - relative frequency of RR intervals discretized into B bins
    ...
    features = [m_RR sd_RR  mean_IntS1 sd_IntS1  mean_IntS2 sd_IntS2  mean_IntSys sd_IntSys  mean_IntDia sd_IntDia m_Ratio_SysRR sd_Ratio_SysRR m_Ratio_DiaRR sd_Ratio_DiaRR m_Ratio_SysDia sd_Ratio_SysDia m_Amp_SysS1 sd_Amp_SysS1 m_Amp_DiaS2 sd_Amp_DiaS2 rmssd_RR pNNx_RR sdsd_RR rhoGTx entr];
else
